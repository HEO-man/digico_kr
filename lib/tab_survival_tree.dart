...
class SurvivalTreeTab extends StatefulWidget {
  final ValueChanged<int>? onLevelChanged;
  const SurvivalTreeTab({super.key, this.onLevelChanged});

  @override
  State<SurvivalTreeTab> createState() => _SurvivalTreeTabState();
}

class _SurvivalTreeTabState extends State<SurvivalTreeTab> {
  ...
  Future<void> _loadTalents() async {
    final url = 'https://heo-man.github.io/digimon-codex-kr/data/talents_alive.json';
    final response = await http.get(Uri.parse(url));

    print('🔎 요청 주소: $url');
    print('📦 응답 코드: ${response.statusCode}');
    print('📄 응답 본문 앞 200자: ${response.body.substring(0, response.body.length > 200 ? 200 : response.body.length)}');

    if (response.statusCode == 200) {
      try {
        final decoded = jsonDecode(response.body);
        print('✅ JSON 파싱 성공: ${decoded.length}개 항목');
        setState(() {
          talents = decoded;
        });
      } catch (e) {
      ...
      print('❗데이터 로드 실패: statusCode=${response.statusCode}');
    }
  }
        ...
        children: [
          ...
          Builder(builder: (_) {
            if (talent != null && talent["id"] != null) {
              final level = localTalentLevels['survival::${talent["id"]}'] ?? 0;
              return Text(
                '$level/${(row % 2 == 1) ? 3 : 5}',
                ...
                TextButton(
                  onPressed: () {
                    ...
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      setState(() {
                        localTalentLevels['survival::${talent["id"]}'] = level;
                      });
